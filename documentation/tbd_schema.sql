...
CREATE TABLE IF NOT EXISTS public.activity_available_date (
    ...
    status VARCHAR(20) DEFAULT 'open' -- Expected values: open, closed, canceled
);
...
-- This index will need to be modified as start_date is not defined in the activities table
...
-- Add available_date_id reference to Reservations table
...
CREATE INDEX idx_master_guide_activity_monitor_guide_id ON master_guide_activity_monitor(master_guide_id);
CREATE INDEX idx_master_guide_activity_inactive ON master_guide_activity_monitor(inactive_status);
